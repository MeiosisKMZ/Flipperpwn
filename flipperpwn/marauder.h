...
typedef enum {
    ...
    FPwnMarauderStateScanAll,
    FPwnMarauderStateJoined,
    FPwnMarauderStatePingScan,
    FPwnMarauderStateArpScan,
    ...
    FPwnMarauderStateSniffRaw,
    FPwnMarauderStateSniffBeacon,
    FPwnMarauderStateSniffDeauth, /* deauthentication frame sniffing */
    FPwnMarauderStateSniffProbe, /* sniffing probe requests */
    FPwnMarauderStateSniffSae,
    FPwnMarauderStateMacTrack,
} FPwnMarauderState;
...
typedef struct {
    ...
    uint16_t marauder_index; /* original AP index from Marauder, if known */
} FPwnWifiAP;
...
/* Begin a combined AP + station scan (raw output to log). */
...
/* Send an ARP sweep of the current subnet (raw output to log). */
...
/* Sniff raw 802.11 traffic (raw output to log). */
void fpwn_marauder_sniff_raw(FPwnMarauder* m);

/* Sniff 802.11 beacon frames (raw output to log). */
void fpwn_marauder_sniff_beacon(FPwnMarauder* m);

/* Sniff 802.11 deauthentication frames (raw output to log). */
void fpwn_marauder_sniff_deauth(FPwnMarauder* m);

/* Sniff SAE commit traffic (raw output to log). */
...
/* Track MACs over time (raw output to log). */
