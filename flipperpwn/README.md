...
| `WIFI_SCAN_ALL` | Combined AP + station scan on the ESP32 |
| `WIFI_JOIN` | Connect ESP32 to a target network |
| `WIFI_DEAUTH` | Deauthentication attack using the current Marauder target selection |
| `WIFI_ARP_SCAN` | Launch an ARP sweep on the joined network |
| `WIFI_SNIFF_RAW` | Raw 802.11 capture to the WiFi status log |
| `WIFI_SNIFF_BEACON` | Sniff beacon frames |
| `WIFI_SNIFF_DEAUTH` | Sniff deauthentication frames |
| `WIFI_SNIFF_SAE` | Sniff SAE commit traffic |
| `WIFI_MAC_TRACK` | Track observed MAC addresses over time |
...
| `WIFI_SCAN_ALL` | Trigger a combined AP + station scan |
| `WIFI_JOIN <ssid> <pass>` | Connect to a network |
| `WIFI_DEAUTH` | Send deauth frames using the current Marauder target selection |
...
| `WIFI_ARP_SCAN` | Run an ARP scan on the joined network |
| `WIFI_SNIFF_PMKID` | Capture PMKID handshakes |
| `WIFI_HANDSHAKE` | WPA handshake capture via deauth |
| `WIFI_SNIFF_RAW [ms]` | Raw 802.11 sniff, optional timed stop |
| `WIFI_SNIFF_BEACON [ms]` | Beacon sniff, optional timed stop |
| `WIFI_SNIFF_DEAUTH [ms]` | Deauth sniff, optional timed stop |
| `WIFI_SCAN_STA` | Scan associated client stations |
| `WIFI_PROBE <ms>` | Sniff probe requests |
| `WIFI_SNIFF_SAE [ms]` | SAE sniff, optional timed stop |
| `WIFI_MAC_TRACK [ms]` | MAC tracker, optional timed stop |
...
| `PING_SCAN` | ICMP sweep of the ESP32's joined subnet |
