...
#define TAG "FPwn"
#define FPWN_INVALID_MARAUDER_AP_INDEX 0xFFFFU
...
static bool ap_identity_matches(const FPwnWifiAP* a, const FPwnWifiAP* b) {
    if(a->bssid[0] != '\0' && b->bssid[0] != '\0') {
        return strcmp(a->bssid, b->bssid) == 0;
    }

    return (a->channel == b->channel) && (strcmp(a->ssid, b->ssid) == 0);
}

static void upsert_ap(FPwnMarauder* m, const FPwnWifiAP* ap) {
    for(uint32_t i = 0; i < m->ap_count; i++) {
        if(ap_identity_matches(&m->aps[i], ap)) {
            FPwnWifiAP merged = *ap;
            if(merged.bssid[0] == '\0' && m->aps[i].bssid[0] != '\0') {
                strncpy(merged.bssid, m->aps[i].bssid, sizeof(merged.bssid) - 1);
                merged.bssid[sizeof(merged.bssid) - 1] = '\0';
            }
            if(merged.marauder_index == FPWN_INVALID_MARAUDER_AP_INDEX) {
                merged.marauder_index = m->aps[i].marauder_index;
            }
            m->aps[i] = merged;
            return;
        }
...
static bool parse_ap_line(const char* line, FPwnWifiAP* ap) {
    ...
    if(!p || !*p) return false;
    ap->marauder_index = (uint16_t)atoi(idx_buf);
...
/*
 * Parse live AP scan lines emitted by Marauder v1.x / hardware v6.x:
 *   <RSSI> Ch: <channel> <BSSID> ESSID: <SSID>
 * Example:
 *   -65 Ch: 6 AA:BB:CC:DD:EE:FF ESSID: MyNetwork
 */
static bool parse_live_ap_line(const char* line, FPwnWifiAP* ap) {
    int rssi = 0;
    unsigned channel = 0;
    char bssid[18] = {0};
    int consumed = 0;

    if(sscanf(line, "%d Ch: %u %17s ESSID: %n", &rssi, &channel, bssid, &consumed) != 3) {
        return false;
    }

    if(channel == 0 || consumed <= 0) return false;

    ap->rssi = (int8_t)rssi;
    ap->channel = (uint8_t)channel;
    ap->marauder_index = FPWN_INVALID_MARAUDER_AP_INDEX;
    ...
    while(ssid_len > 0 &&
          (ap->ssid[ssid_len - 1] == ' ' || ap->ssid[ssid_len - 1] == '\r' ||
           ap->ssid[ssid_len - 1] == '\n')) {
        ap->ssid[--ssid_len] = '\0';
    }
...
static bool parse_list_ap_line(const char* line, FPwnWifiAP* ap) {
    ...
    char idx_buf[8];
    size_t idx_len = (size_t)(idx_end - (line + 1));
    if(idx_len == 0 || idx_len > sizeof(idx_buf) - 1) return false;
    memcpy(idx_buf, line + 1, idx_len);
    idx_buf[idx_len] = '\0';
    ap->marauder_index = (uint16_t)atoi(idx_buf);
    const char* p = idx_end + 1;

    /* Marauder v1.x: [0][CH:6] SSID -45 (selected) */
    if(strncmp(p, "[CH:", 4) == 0) {
        p += 4;
        ap->channel = (uint8_t)atoi(p);

        const char* ch_end = strchr(p, ']');
        if(!ch_end) return false;
        p = ch_end + 1;
        while(*p == ' ')
            p++;
        if(!*p) return false;

        const char* selected = strstr(p, " (selected)");
        const char* end = selected ? selected : line + strlen(line);
        const char* rssi_start = find_prev_token_start(p, end);
        if(rssi_start <= p) return false;

        char rssi_buf[8];
        size_t rssi_len = (size_t)(end - rssi_start);
        while(rssi_len > 0 && (rssi_start[rssi_len - 1] == ' ' || rssi_start[rssi_len - 1] == ')'))
            rssi_len--;
        if(rssi_len == 0) return false;
        if(rssi_len > sizeof(rssi_buf) - 1) rssi_len = sizeof(rssi_buf) - 1;
        memcpy(rssi_buf, rssi_start, rssi_len);
    ...
    /* Legacy format fallback: [0] SSID (-45) ch:6 [WPA2] AA:BB:... */
    while(*p == ' ')
        p++;
    if(!*p) return false;
...
static uint16_t get_command_ap_index(FPwnMarauder* m, uint8_t ap_idx) {
    uint16_t resolved = ap_idx;

    furi_mutex_acquire(m->mutex, FuriWaitForever);
    if(ap_idx < m->ap_count &&
       m->aps[ap_idx].marauder_index != FPWN_INVALID_MARAUDER_AP_INDEX) {
        resolved = m->aps[ap_idx].marauder_index;
    }
...
static uint8_t get_ap_channel(FPwnMarauder* m, uint8_t ap_idx) {
    uint8_t channel = 0;

    furi_mutex_acquire(m->mutex, FuriWaitForever);
    if(ap_idx < m->ap_count) {
        channel = m->aps[ap_idx].channel;
    }
    furi_mutex_release(m->mutex);
...
static void fpwn_marauder_rx_cb(const char* line, void* ctx) {
    ...
    case FPwnMarauderStateScanStopping: {
        ...
        if(parse_ap_line(line, &ap) || parse_live_ap_line(line, &ap)) {
            upsert_ap(m, &ap);
            FURI_LOG_I(TAG, "AP: %s %s", ap.ssid, ap.bssid);
        } else if(parse_list_ap_line(line, &ap)) {
            upsert_ap(m, &ap);
            FURI_LOG_I(TAG, "AP(list): %s %s", ap.ssid, ap.bssid);
        } else {
    ...
    case FPwnMarauderStateScanAll: {
        if(line_has_done_marker(line) || strncmp(line, "Scanning", 8) == 0 ||
           strncmp(line, "Stopping", 8) == 0 || strstr(line, "Started")) {
            if(line_has_done_marker(line)) {
                m->state = FPwnMarauderStateIdle;
                FURI_LOG_I(
                    TAG,
                    "scanall complete, %lu APs / %lu stations",
                    (unsigned long)m->ap_count,
                    (unsigned long)m->station_count);
            }
            break;
        }
        ...
        if(parse_ap_line(line, &ap) || parse_live_ap_line(line, &ap) || parse_list_ap_line(line, &ap)) {
            upsert_ap(m, &ap);
        } else if(parse_station_line(line, &sta)) {
    ...
    case FPwnMarauderStateSniffRaw:
    case FPwnMarauderStateSniffBeacon:
    case FPwnMarauderStateSniffSae:
    case FPwnMarauderStateMacTrack:
    case FPwnMarauderStateArpScan:
        /* Raw scan/attack output — no structured parsing, output goes to log
...
void fpwn_marauder_scan_all(FPwnMarauder* m) {
    furi_assert(m);
    ...
    m->station_count = 0;
    m->state = FPwnMarauderStateScanAll;
    furi_mutex_release(m->mutex);

    fpwn_wifi_uart_send(m->uart, "scanall");
    FURI_LOG_I(TAG, "scanall started");
}
...
void fpwn_marauder_join(FPwnMarauder* m, uint8_t ap_idx, const char* password) {
    ...
    char cmd[128];
    uint16_t command_ap_idx = get_command_ap_index(m, ap_idx);

    if(password[0] != '\0') {
        snprintf(cmd, sizeof(cmd), "join -a %u -p %s", (unsigned)command_ap_idx, password);
    } else {
        snprintf(cmd, sizeof(cmd), "join -a %u", (unsigned)command_ap_idx);
    }
    ...
    FURI_LOG_I(
        TAG, "join AP local=%u command=%u", (unsigned)ap_idx, (unsigned)command_ap_idx);
}
...
void fpwn_marauder_arp_scan(FPwnMarauder* m) {
    furi_assert(m);

    furi_mutex_acquire(m->mutex, FuriWaitForever);
    memset(m->hosts, 0, sizeof(m->hosts));
    m->host_count = 0;
    m->state = FPwnMarauderStateArpScan;
    furi_mutex_release(m->mutex);

    fpwn_wifi_uart_send(m->uart, "arpscan");
    FURI_LOG_I(TAG, "arpscan started");
}
...
void fpwn_marauder_evil_portal(FPwnMarauder* m, const char* ssid) {
    furi_assert(m);
    /* Marauder v1.x expects a preloaded SSID list, then "evilportal -c start". */
    char cmd[128];
    fpwn_wifi_uart_send(m->uart, "clearlist -s");
    furi_delay_ms(100);
    snprintf(cmd, sizeof(cmd), "ssid -a -n %s", ssid);
    fpwn_wifi_uart_send(m->uart, cmd);
    furi_delay_ms(100);
    fpwn_wifi_uart_send(m->uart, "evilportal -c start");
...
void fpwn_marauder_sniff_raw(FPwnMarauder* m) {
    furi_assert(m);
    fpwn_wifi_uart_send(m->uart, "sniffraw");
    furi_mutex_acquire(m->mutex, FuriWaitForever);
    m->state = FPwnMarauderStateSniffRaw;
    furi_mutex_release(m->mutex);
    FURI_LOG_I(TAG, "raw sniff started");
}

void fpwn_marauder_sniff_beacon(FPwnMarauder* m) {
    furi_assert(m);
    fpwn_wifi_uart_send(m->uart, "sniffbeacon");
    furi_mutex_acquire(m->mutex, FuriWaitForever);
    m->state = FPwnMarauderStateSniffBeacon;
    furi_mutex_release(m->mutex);
    FURI_LOG_I(TAG, "beacon sniff started");
}
...
void fpwn_marauder_sniff_deauth(FPwnMarauder* m) {
    ...
    furi_mutex_release(m->mutex);
    FURI_LOG_I(TAG, "deauth sniff started");
}

void fpwn_marauder_sniff_sae(FPwnMarauder* m) {
    furi_assert(m);
    fpwn_wifi_uart_send(m->uart, "sniffsae");
    furi_mutex_acquire(m->mutex, FuriWaitForever);
    m->state = FPwnMarauderStateSniffSae;
    furi_mutex_release(m->mutex);
    FURI_LOG_I(TAG, "sae sniff started");
}

void fpwn_marauder_sniff_handshake(FPwnMarauder* m) {
    furi_assert(m);
    fpwn_wifi_uart_send(m->uart, "sniffpmkid -d");
    furi_mutex_acquire(m->mutex, FuriWaitForever);
    m->state = FPwnMarauderStateSniffPmkid;
    furi_mutex_release(m->mutex);
    FURI_LOG_I(TAG, "handshake capture via deauth started");
}
...
void fpwn_marauder_select_ap(FPwnMarauder* m, uint8_t ap_idx) {
    ...
    char cmd[32];
    uint16_t command_ap_idx = get_command_ap_index(m, ap_idx);
    snprintf(cmd, sizeof(cmd), "select -a %u", (unsigned)command_ap_idx);
    fpwn_wifi_uart_send(m->uart, cmd);
    FURI_LOG_I(
        TAG,
        "selected AP local=%u command=%u",
        (unsigned)ap_idx,
        (unsigned)command_ap_idx);
}
...
void fpwn_marauder_mac_track(FPwnMarauder* m) {
    furi_assert(m);
    fpwn_wifi_uart_send(m->uart, "mactrack");
    furi_mutex_acquire(m->mutex, FuriWaitForever);
    m->state = FPwnMarauderStateMacTrack;
    furi_mutex_release(m->mutex);
    FURI_LOG_I(TAG, "mac track started");
}
...
void fpwn_marauder_sniff_handshake_targeted(FPwnMarauder* m, uint8_t ap_idx) {
    furi_assert(m);

    fpwn_marauder_select_ap(m, ap_idx);
    furi_delay_ms(200);

    uint8_t channel = get_ap_channel(m, ap_idx);
    char cmd[64];
    if(channel > 0) {
        snprintf(cmd, sizeof(cmd), "sniffpmkid -c %u -d -l", (unsigned)channel);
    } else {
        snprintf(cmd, sizeof(cmd), "sniffpmkid -d -l");
    }
    fpwn_wifi_uart_send(m->uart, cmd);
    ...
    FURI_LOG_I(TAG, "targeted handshake capture AP %u", (unsigned)ap_idx);
}
