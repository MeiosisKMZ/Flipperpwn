...
static void fpwn_wifi_prepare_text_input(FPwnApp* app, const char* header_text) {
    furi_assert(app);
    furi_assert(app->wifi_text_input);

    text_input_reset(app->wifi_text_input);
    text_input_set_header_text(app->wifi_text_input, header_text);
...
typedef enum {
    ...
    FPwnWifiMenuSniffRaw,
    FPwnWifiMenuSniffBeacon,
    FPwnWifiMenuSniffDeauth,
    FPwnWifiMenuSniffProbe, /* sniff probe requests */
    FPwnWifiMenuSniffSae,
    FPwnWifiMenuMacTrack,
        ...
        FPwnWifiScanModel * m,
        {
            ...
            if(event->key == InputKeyUp) {
                ...
                if(m->ap_count > 0 && m->selected_index < m->ap_count) {
                    ...
                    if(app->wifi_deauth_mode) {
                    ...
                    } else if(app->wifi_handshake_mode) {
                        /* Targeted WPA/PMKID capture mode — select AP first */
                        app->wifi_handshake_mode = false;
                        app->wifi_deauth_mode = false;
                        fpwn_marauder_sniff_handshake_targeted(app->marauder, m->selected_index);
                        furi_string_reset(app->wifi_status_text);
                        text_box_reset(app->wifi_status);
                        fpwn_set_current_view(FPwnViewWifiStatus);
                        view_dispatcher_switch_to_view(app->view_dispatcher, FPwnViewWifiStatus);
                        ...
                        fpwn_wifi_prepare_text_input(app, "Password (empty=open)");
...
static void fpwn_scan_timer_cb(void* ctx) {
    ...
    if(state == FPwnMarauderStateScanAll) {
        with_view_model(
            app->wifi_scan_view,
            FPwnWifiScanModel * m,
            {
     ...
     * between timer ticks. */
    {
        ...
        bool still_active = (cur == FPwnMarauderStateScanning ||
                             cur == FPwnMarauderStateScanStopping ||
                             cur == FPwnMarauderStateScanAll);
        with_view_model(
            app->wifi_scan_view,
            FPwnWifiScanModel * m,
            {
...
static void fpwn_wifi_menu_callback(void* ctx, uint32_t index) {
    ...
    case FPwnWifiMenuEvilPortal:
        ...
        memset(app->wifi_text_buf, 0, sizeof(app->wifi_text_buf));
        fpwn_wifi_prepare_text_input(app, "Portal SSID");
        fpwn_set_current_view(FPwnViewWifiPassword);
        view_dispatcher_switch_to_view(app->view_dispatcher, FPwnViewWifiPassword);
        break;
    ...
    case FPwnWifiMenuHandshake:
        /* Scan APs; when user presses OK the input callback starts capture */
        app->wifi_handshake_mode = true;
        app->wifi_deauth_mode = false;
        app->wifi_portal_mode = false;
        with_view_model(
            app->wifi_scan_view,
...
void fpwn_wifi_menu_setup(FPwnApp* app) {
    ...
    submenu_add_item(
        app->wifi_menu, "Scan APs+STA", FPwnWifiMenuScanAll, fpwn_wifi_menu_callback, app);
    submenu_add_item(
    ...
    submenu_add_item(
        app->wifi_menu, "ARP Scan", FPwnWifiMenuArpScan, fpwn_wifi_menu_callback, app);
    submenu_add_item(
    ...
    submenu_add_item(
        app->wifi_menu, "Sniff Raw", FPwnWifiMenuSniffRaw, fpwn_wifi_menu_callback, app);
    submenu_add_item(
        app->wifi_menu, "Sniff Beacon", FPwnWifiMenuSniffBeacon, fpwn_wifi_menu_callback, app);
    submenu_add_item(
        app->wifi_menu, "Sniff Deauth", FPwnWifiMenuSniffDeauth, fpwn_wifi_menu_callback, app);
    submenu_add_item(
        app->wifi_menu, "Sniff Probes", FPwnWifiMenuSniffProbe, fpwn_wifi_menu_callback, app);
    submenu_add_item(
        app->wifi_menu, "Sniff SAE", FPwnWifiMenuSniffSae, fpwn_wifi_menu_callback, app);
    submenu_add_item(
        app->wifi_menu, "MAC Tracker", FPwnWifiMenuMacTrack, fpwn_wifi_menu_callback, app);
    submenu_add_item(
