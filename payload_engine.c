...
#ifndef KEY_MOD_RIGHT_ALT
#define KEY_MOD_RIGHT_ALT 0x4000u
#endif

#ifndef HID_KEYBOARD_NON_US_BACKSLASH
#define HID_KEYBOARD_NON_US_BACKSLASH 0x64u
#endif
...
/**
 * Map a single ASCII character to a HID keycode + modifier mask.
 *
 * This project targets hosts configured with a standard French AZERTY layout.
 * Returns false if the character cannot be mapped.
 */
bool fpwn_char_to_hid(char c, uint16_t* keycode, uint16_t* modifiers) {
    *modifiers = 0;

    /* Lowercase letters */
    switch(c) {
    case 'a':
        *keycode = HID_KEYBOARD_Q;
        return true;
    case 'm':
        *keycode = HID_KEYBOARD_SEMICOLON;
        return true;
    case 'q':
        *keycode = HID_KEYBOARD_A;
        return true;
    case 'w':
        *keycode = HID_KEYBOARD_Z;
        return true;
    case 'z':
        *keycode = HID_KEYBOARD_W;
        return true;
    }
    ...
    case '.':
        ...
        *modifiers = KEY_MOD_LEFT_SHIFT;
        return true;
    case '<':
        *keycode = HID_KEYBOARD_NON_US_BACKSLASH;
        return true;
    case '>':
        *keycode = HID_KEYBOARD_NON_US_BACKSLASH;
        *modifiers = KEY_MOD_LEFT_SHIFT;
...
void fpwn_type_char(char c) {
    ...
    if(!fpwn_char_to_hid(c, &keycode, &modifiers)) {
        FURI_LOG_W(TAG, "No HID mapping for char 0x%02x", (uint8_t)c);
        return;
    }

    furi_hal_hid_kb_press(modifiers | keycode);
    furi_delay_ms(2);
    furi_hal_hid_kb_release(modifiers | keycode);
...
static uint16_t fpwn_named_key(const char* name) {
    ...
    if(name[0] != '\0' && name[1] == '\0') {
        uint16_t kc;
        uint16_t modifiers;
        if(fpwn_char_to_hid(name[0], &kc, &modifiers)) return modifiers | kc;
    }
    /* Named keys */
...
static void fpwn_exec_command(const char* line, FPwnApp* app) {
    ...
    /* WIFI_SCAN_ALL — trigger combined AP + station scan */
    if(strcmp(line, "WIFI_SCAN_ALL") == 0) {
        if(!app->marauder || !app->wifi_uart || !fpwn_wifi_uart_is_connected(app->wifi_uart)) {
            FURI_LOG_W(TAG, "WIFI_SCAN_ALL: ESP32 not connected, skipping");
            return;
        }
        fpwn_marauder_scan_all(app->marauder);
    ...
    /* WIFI_ARP_SCAN */
    if(strcmp(line, "WIFI_ARP_SCAN") == 0) {
        if(!app->marauder || !app->wifi_uart || !fpwn_wifi_uart_is_connected(app->wifi_uart)) {
            FURI_LOG_W(TAG, "WIFI_ARP_SCAN: ESP32 not connected, skipping");
            return;
        }
        fpwn_marauder_arp_scan(app->marauder);
        return;
    }
    ...
    if(strcmp(line, "WIFI_HANDSHAKE") == 0) {
        ...
        fpwn_marauder_sniff_handshake(app->marauder);
        return;
    }

    if(strncmp(line, "WIFI_SNIFF_RAW ", 15) == 0) {
        if(!app->marauder || !app->wifi_uart || !fpwn_wifi_uart_is_connected(app->wifi_uart)) {
            FURI_LOG_W(TAG, "WIFI_SNIFF_RAW: ESP32 not connected, skipping");
            return;
        }
        uint32_t duration_ms = (uint32_t)atoi(line + 15);
        if(duration_ms > 120000) duration_ms = 120000;
        fpwn_marauder_sniff_raw(app->marauder);
        uint32_t start = furi_get_tick();
    ...
    if(strcmp(line, "WIFI_SNIFF_RAW") == 0) {
        if(!app->marauder || !app->wifi_uart || !fpwn_wifi_uart_is_connected(app->wifi_uart)) {
            FURI_LOG_W(TAG, "WIFI_SNIFF_RAW: ESP32 not connected, skipping");
            return;
        }
    ...
    if(strncmp(line, "WIFI_SNIFF_BEACON ", 18) == 0) {
        if(!app->marauder || !app->wifi_uart || !fpwn_wifi_uart_is_connected(app->wifi_uart)) {
            FURI_LOG_W(TAG, "WIFI_SNIFF_BEACON: ESP32 not connected, skipping");
            return;
        }
        uint32_t duration_ms = (uint32_t)atoi(line + 18);
        if(duration_ms > 120000) duration_ms = 120000;
        fpwn_marauder_sniff_beacon(app->marauder);
        uint32_t start = furi_get_tick();
        while(!app->abort_requested && (furi_get_tick() - start) < furi_ms_to_ticks(duration_ms)) {
            furi_delay_ms(100);
        }
    ...
    if(strcmp(line, "WIFI_SNIFF_BEACON") == 0) {
        if(!app->marauder || !app->wifi_uart || !fpwn_wifi_uart_is_connected(app->wifi_uart)) {
            FURI_LOG_W(TAG, "WIFI_SNIFF_BEACON: ESP32 not connected, skipping");
            return;
        }
    ...
    if(strncmp(line, "WIFI_SNIFF_DEAUTH ", 18) == 0) {
        if(!app->marauder || !app->wifi_uart || !fpwn_wifi_uart_is_connected(app->wifi_uart)) {
            FURI_LOG_W(TAG, "WIFI_SNIFF_DEAUTH: ESP32 not connected, skipping");
            return;
        }
        uint32_t duration_ms = (uint32_t)atoi(line + 18);
        if(duration_ms > 120000) duration_ms = 120000;
    ...
    if(strcmp(line, "WIFI_SNIFF_DEAUTH") == 0) {
        if(!app->marauder || !app->wifi_uart || !fpwn_wifi_uart_is_connected(app->wifi_uart)) {
            FURI_LOG_W(TAG, "WIFI_SNIFF_DEAUTH: ESP32 not connected, skipping");
            return;
        }
    ...
    if(strncmp(line, "WIFI_SNIFF_SAE ", 15) == 0) {
        if(!app->marauder || !app->wifi_uart || !fpwn_wifi_uart_is_connected(app->wifi_uart)) {
            FURI_LOG_W(TAG, "WIFI_SNIFF_SAE: ESP32 not connected, skipping");
            return;
        }
        uint32_t duration_ms = (uint32_t)atoi(line + 15);
        if(duration_ms > 120000) duration_ms = 120000;
        fpwn_marauder_sniff_sae(app->marauder);
        uint32_t start = furi_get_tick();
        while(!app->abort_requested && (furi_get_tick() - start) < furi_ms_to_ticks(duration_ms)) {
            furi_delay_ms(100);
        }
    ...
    if(strcmp(line, "WIFI_SNIFF_SAE") == 0) {
        if(!app->marauder || !app->wifi_uart || !fpwn_wifi_uart_is_connected(app->wifi_uart)) {
            FURI_LOG_W(TAG, "WIFI_SNIFF_SAE: ESP32 not connected, skipping");
            return;
        }
    ...
    if(strncmp(line, "WIFI_MAC_TRACK ", 15) == 0) {
        if(!app->marauder || !app->wifi_uart || !fpwn_wifi_uart_is_connected(app->wifi_uart)) {
            FURI_LOG_W(TAG, "WIFI_MAC_TRACK: ESP32 not connected, skipping");
            return;
        }
        uint32_t duration_ms = (uint32_t)atoi(line + 15);
        if(duration_ms > 120000) duration_ms = 120000;
        fpwn_marauder_mac_track(app->marauder);
        uint32_t start = furi_get_tick();
        while(!app->abort_requested && (furi_get_tick() - start) < furi_ms_to_ticks(duration_ms)) {
            furi_delay_ms(100);
        }
    ...
    if(strcmp(line, "WIFI_MAC_TRACK") == 0) {
        if(!app->marauder || !app->wifi_uart || !fpwn_wifi_uart_is_connected(app->wifi_uart)) {
            FURI_LOG_W(TAG, "WIFI_MAC_TRACK: ESP32 not connected, skipping");
            return;
        }
